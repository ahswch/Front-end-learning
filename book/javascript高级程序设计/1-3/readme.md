...
- 位操作符
  ...
  - 按位或（OR）
    - 由一个竖线符号（|）表示,只有在两个位都是 0的情况下才返回 0
  - 按位异或（XOR）
    - 由一个插入符号（^）表示,只有一个 1时(对应位不同)才返回 1
  - 左移
    - 由两个小于号（<<）表示，将数值的所有位向左移动指定的位数，左移操作会以 0来填充这些空位
  - 有符号的右移
    - 由两个大于号（>>）表示，将数值向右移动，但保留符号位，用符号位的值来填充所有空位
  - 无符号右移
    - 由 3个大于号（>>>）表示，将数值的所有 32位都向右移动，无符号右移是以 0来填充空位
- 布尔操作符(p44)
  > 非（NOT）、与（AND） 和或（OR）
  - 逻辑非
    - 由一个叹号（！）表示,而同时使用两个逻辑非操作符，实际上就会模拟 Boolean()转型函数的行为
  - 逻辑与
    - 由两个和号（&&）表示
  - 逻辑或
    - 由两个竖线符号（||）表示
...
- 加性操作符
  ...
  - 减法-
- 关系操作符
  > 小于（<）、大于（>）、小于等于（<=）和大于等于（>=）这几个关系操作符用于对两个值进行比较,返回布尔值
- 相等操作符
  - 相等和不相等（两个操作符都会先转换操作数（通常称为强制转型），再比较它们的相等性）
    - 相等操作符由两个等于号（==）表示，两个操作数相等，则返回 true
    - 不相等操作符由叹号后跟等于号（!=）表示，两个操作数不相等，则返回 true
  - 全等和不全等
    - 全等操作符由 3个等于号（===）表示，只在两个操作数未经转换就相等的情况下返回 true
    - 不全等操作符由一个叹号后跟两个等于号（!==）表示，两个操作数未经转换就不相等的情况下返回 true
- 条件操作符
  ...
  ```
- 赋值操作符
  - 简单的赋值操作符由等于号（=）表示,把右侧的值赋给左侧的变量
  - 在等于号（=）前面再添加乘性操作符、加性操作符或位操作符,完成复合赋值操作(*=、/=、%=、+=、-=、<<=、>>=、>>>=)
- 逗号操作符
  - 声明多个变量
